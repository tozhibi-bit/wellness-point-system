...
export default async function EmployeePage() {
  ...
  const monthTx = transactions.filter((t) => t.usedDate.toISOString().startsWith(yearMonth));
  const monthTotal = monthTx
    .filter((t) => t.status !== "pending_usage" && t.status !== "cancelled")
    .reduce((s, t) => s + t.serviceAmountYen, 0);
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20, marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gap: 20 }}>
            <div style={styles.card}>
              <div style={statLabelStyle}>今月の利用額(合計)</div>
              <div style={statValueStyle}>{yen(monthTotal)}</div>
              <div style={statDeltaStyle}>{points.used} pt 使用済み</div>
            </div>
            <div style={styles.card}>
              <div style={statLabelStyle}>今月の利用件数</div>
              <div style={statValueStyle}>
                {monthTx.length}
                <span style={{ fontSize: 14, color: "var(--ink-mute)", marginLeft: 4, fontWeight: 400 }}>件</span>
              </div>
              <div style={statDeltaStyle}>
                確定済 {monthTx.filter((t) => t.status === "confirmed" || t.status === "invoiced").length} 件
              </div>
            </div>
          </div>
        </div>
...
const statDeltaStyle: React.CSSProperties = {
  fontSize: 11,
  color: "var(--matcha)",
  marginTop: 6,
  fontFamily: "'JetBrains Mono', monospace",
